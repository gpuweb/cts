...
export function linearRange(a: Scalar, b: Scalar, num_steps: Scalar): Array<number> {
  const { f32_a, f32_b, u32_num_steps } = unwrapRangeParams(a, b, num_steps);

  return Array.from(Array(u32_num_steps).keys()).map(i =>
    lerp(f32_a, f32_b, i / (u32_num_steps - 1))
  );
}
 ...
 * */
export function biasedRange(a: Scalar, b: Scalar, num_steps: Scalar): Array<number> {
  const c = 2;
  const { f32_a, f32_b, u32_num_steps } = unwrapRangeParams(a, b, num_steps);

  return Array.from(Array(u32_num_steps).keys()).map(i =>
    lerp(f32_a, f32_b, Math.pow(lerp(0, 1, i / (u32_num_steps - 1)), c))
  );
}
