...
import { Type, TypeF32, Value } from '../../../../../util/conversion.js';
...
export function runDerivativeTest(
  ...
  let type: Type = TypeF32;
  if (vectorize !== undefined) {
    const packed = packScalarsToVector([TypeF32, TypeF32], TypeF32, cases, vectorize);
    cases = packed.cases;
      ...
      for (let i = 0; i < cases.length; i++) {
        ...
        for (let d = 0; d < 2; d++) {
          ...
          if (!cmp.matched) {
            ...
            // one of the two derivatives and return that results to all of the invocations.
