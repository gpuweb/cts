...
import { allInputSources, Case, makeVectorToF32IntervalCase, run } from '../../expression.js';
...
const makeCaseF32 = (x: number[]): Case => {
...
g.test('f32_vec2')
  ...
  .fn(async t => {
    const cases: Case[] = kVectorTestValues[2].map(makeCaseF32);
...
g.test('f32_vec3')
  ...
  .params(u => u.combine('inputSource', allInputSources))
  .fn(async t => {
    const cases: Case[] = kVectorTestValues[3].map(makeCaseF32);
...
g.test('f32_vec4')
  ...
  .params(u => u.combine('inputSource', allInputSources))
  .fn(async t => {
    const cases: Case[] = kVectorTestValues[4].map(makeCaseF32);
