...
/** Line format af_data_gen/main.cpp outputs */
...
// prettier-ignore
export const addition_raw_values: RawBinaryData[] = [
...
const addition_table: Record<number, FPInterval> = {};
addition_raw_values.forEach(value => {
  addition_table[hashRawDataInputs(value.lhs, value.rhs)] = FP.abstract.spanIntervals(
    ...value.expected.map(e => FP.abstract.correctlyRoundedInterval(e))
  );
});
...
export function getAdditionAFInterval(lhs: number, rhs: number): FPInterval {
  return addition_table[hashRawDataInputs(lhs, rhs)];
}
