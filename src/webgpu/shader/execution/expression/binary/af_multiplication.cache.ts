...
const multiplicationVectorScalarInterval = (v: readonly number[], s: number): FPVector => {
  return FP.abstract_float.toVector(v.map(e => FP.abstract_float.multiplicationInterval(e, s)));
};

const multiplicationScalarVectorInterval = (s: number, v: readonly number[]): FPVector => {
  return FP.abstract_float.toVector(v.map(e => FP.abstract_float.multiplicationInterval(s, e)));
};
...
const scalar_cases = {
  ['scalar']: () => {
    return FP.abstract_float.generateScalarPairToIntervalCases(
      ...
      sparseScalarF64Range(),
      'finite',
      FP.abstract_float.multiplicationInterval
    );
  },
};
