/**
 * Code used to generate manual values for 'correctly rounded' AbstractFloat tests in the CTS.
 *
 * These are generated in a C++ program, because it allows for easy access to higher than 64-bit floating point numbers
 * (specifically 128-bit), which allows for calculating roundings when infinitely precise calculcations are not
 * precisely representable in 64-bit floats. This gets around the fact that numbers in Typescript are internally
 * 64-bits, thus making it difficult to detect when rounding occurs for AbstractFloats without importing a higher
 * precision floating point library.
 *
 * This codes is not meant to be automatically built/used by the CTS, but instead is a reference for how the values in
 * af_data.ts were generated
 */
#include <cassert>
#include <cstdint>
#include <iostream>
...
/**
 * Mapping from Numeric value -> TS representation, should include all the values that appear in kInterestingF64Values
 * in math.ts
 */
...
/** Print out a string representation of a specific value that can be copied in a CTS test */
std::string printAbstractFloat(const double val) {
    ...
    std::stringstream ss;
    const std::string val_str = std::to_string(val);
...
/**
 * Generate the 64-bit float interval that a higher precision value will quantized down to.
 * If the value if exactly representable in 64-bit floating point this will be a singular value, otherwise it will be
 * the two 64-bit values nearest to the value.
 *
 * This is done via manipulating the global process rounding mode, thus this code is non-rentrant, so should not be used
 * in concurrent/asynchronous processing
 */
...
/** Generate a string for a binary operation that can be copied into a CTS test file */
...
/** Function that performs a binary operation, i.e. addition, subtraction, etc */
...
/** Generate a string, that can be copied into a CTS test file, for all of the tests cases for a binary operation  */
std::string printBinaryOpCases(const BinaryOp op, const std::string& name) {
    ...
    for (const auto& [lhs, lhs_str] : kInterestingF64s) {
        for (const auto& [rhs, rhs_str] : kInterestingF64s) {
            ...
            // CTS needs to consider that subnormals may be flushed to zero at any point, so applying potential
            // flushings to get additional results.
...
int main() {
    ...
    {
        // Confirm that calculating f64 imprecise results generates two possible roundings.
        const auto [begin, end] = quantizeToAbstractFloat(static_cast<long double>(0.1) * static_cast<long double>(0.1));
