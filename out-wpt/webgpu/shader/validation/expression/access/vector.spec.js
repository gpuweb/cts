...
const kAbstractCases = {
  ...
  literal_2: { wgsl: 'const r = V[2];', ok: (width) => width > 2 },
  literal_3: { wgsl: 'const r = V[3];', ok: (width) => width > 3 },
  literal_0i: { wgsl: 'const r = V[0i];', ok: true },
  literal_1i: { wgsl: 'const r = V[1i];', ok: true },
  literal_2i: { wgsl: 'const r = V[2i];', ok: (width) => width > 2 },
  literal_3i: { wgsl: 'const r = V[3i];', ok: (width) => width > 3 },
  literal_0u: { wgsl: 'const r = V[0u];', ok: true },
  literal_1u: { wgsl: 'const r = V[1u];', ok: true },
  literal_2u: { wgsl: 'const r = V[2u];', ok: (width) => width > 2 },
  literal_3u: { wgsl: 'const r = V[3u];', ok: (width) => width > 3 },

  // indexing with 'const' variable
  const_0: { wgsl: 'const i = 0; const r = V[i];', ok: true },
  const_1: { wgsl: 'const i = 1; const r = V[i];', ok: true },
  const_2: { wgsl: 'const i = 2; const r = V[i];', ok: (width) => width > 2 },
  const_3: { wgsl: 'const i = 3; const r = V[i];', ok: (width) => width > 3 },
  const_0i: { wgsl: 'const i = 0i; const r = V[i];', ok: true },
  const_1i: { wgsl: 'const i = 1i; const r = V[i];', ok: true },
  const_2i: { wgsl: 'const i = 2i; const r = V[i];', ok: (width) => width > 2 },
  const_3i: { wgsl: 'const i = 3i; const r = V[i];', ok: (width) => width > 3 },
  const_0u: { wgsl: 'const i = 0u; const r = V[i];', ok: true },
  const_1u: { wgsl: 'const i = 1u; const r = V[i];', ok: true },
  const_2u: { wgsl: 'const i = 2u; const r = V[i];', ok: (width) => width > 2 },
  const_3u: { wgsl: 'const i = 3u; const r = V[i];', ok: (width) => width > 3 },
  ...
  const_expr_2: { wgsl: 'const r = V[4 / 2];', ok: (width) => width > 2 },
  const_expr_3: { wgsl: 'const r = V[6 / 2];', ok: (width) => width > 3 },
  const_expr_2_via_trig: {
  ...
  z: { wgsl: 'const r = V.z;', ok: (width) => width > 2 },
  w: { wgsl: 'const r = V.w;', ok: (width) => width > 3 },
  r: { wgsl: 'const r = V.r;', ok: true },
  g: { wgsl: 'const r = V.g;', ok: true },
  b: { wgsl: 'const r = V.b;', ok: (width) => width > 2 },
  a: { wgsl: 'const r = V.a;', ok: (width) => width > 3 },

  // swizzles
  xy: { wgsl: 'const r = V.xy;', ok: true },
  yx: { wgsl: 'const r = V.yx;', ok: true },
  xyx: { wgsl: 'const r = V.xyx;', ok: true },
  xyz: { wgsl: 'const r = V.xyz;', ok: (width) => width > 2 },
  zyx: { wgsl: 'const r = V.zyx;', ok: (width) => width > 2 },
  xyxy: { wgsl: 'const r = V.xyxy;', ok: true },
  xyxz: { wgsl: 'const r = V.xyxz;', ok: (width) => width > 2 },
  xyzw: { wgsl: 'const r = V.xyzw;', ok: (width) => width > 3 },
  yxwz: { wgsl: 'const r = V.yxwz;', ok: (width) => width > 3 },
  rg: { wgsl: 'const r = V.rg;', ok: true },
  gr: { wgsl: 'const r = V.gr;', ok: true },
  rgg: { wgsl: 'const r = V.rgg;', ok: true },
  rgb: { wgsl: 'const r = V.rgb;', ok: (width) => width > 2 },
  grb: { wgsl: 'const r = V.grb;', ok: (width) => width > 2 },
  rgbr: { wgsl: 'const r = V.rgbr;', ok: (width) => width > 2 },
  rgba: { wgsl: 'const r = V.rgba;', ok: (width) => width > 3 },
  gbra: { wgsl: 'const r = V.gbra;', ok: (width) => width > 3 },

  // swizzle chains
  xy_yx: { wgsl: 'const r = V.xy.yx;', ok: true },
  xyx_xxy: { wgsl: 'const r = V.xyx.xxy;', ok: true },
  xyz_zyx: { wgsl: 'const r = V.xyz.zyx;', ok: (width) => width > 2 },
  xyxy_rrgg: { wgsl: 'const r = V.xyxy.rrgg;', ok: true },
  rbrg_xyzw: { wgsl: 'const r = V.rbrg.xyzw;', ok: (width) => width > 2 },
  xyxz_rbg_yx: { wgsl: 'const r = V.xyxz.rbg.yx;', ok: (width) => width > 2 },
  wxyz_bga_xy: { wgsl: 'const r = V.wxyz.bga.xy;', ok: (width) => width > 3 },
...
u.
combine('vector_width', [2, 3, 4]).
combine('element_type', ['int', 'float']).
beginSubcases().
